...
/* L10: */
			...
			if (nounit) {
			    ...
			    z__1.r = X(j).r * A(kplus1,j).r - X(j).i * A(kplus1,j).i, z__1.i = X(j).r * A(kplus1,j).i + 
				    X(j).i * A(kplus1,j).r;
...
/* L30: */
			...
			if (nounit) {
			    ...
			    z__1.r = X(jx).r * A(kplus1,j).r - X(jx).i * A(kplus1,j).i, z__1.i = X(jx).r * A(kplus1,j).i + 
				    X(jx).i * A(kplus1,j).r;
...
/* L50: */
			...
			if (nounit) {
			    ...
			    i__1 = j;
			    i__3 = j * a_dim1 + 1;
			    z__1.r = X(j).r * A(1,j).r - X(j).i * A(1,j).i, z__1.i = X(j).r * A(1,j).i + 
				    X(j).i * A(1,j).r;
			    X(j).r = z__1.r, X(j).i = z__1.i;
			}
		    }
...
/* L70: */
			...
			if (nounit) {
			    ...
			    i__4 = jx;
			    i__1 = j * a_dim1 + 1;
			    z__1.r = X(jx).r * A(1,j).r - X(jx).i * A(1,j).i, z__1.i = X(jx).r * A(1,j).i + 
				    X(jx).i * A(1,j).r;
			    X(jx).r = z__1.r, X(jx).i = z__1.i;
			}
		    }
...
/*        Form  x := A'*x  or  x := conjg( A' )*x. */
	...
	if (lsame_(uplo, "U")) {
		...
		for (j = *n; j >= 1; --j) {
			...
			if (nounit) {
			    i__3 = kplus1 + j * a_dim1;
			    z__1.r = temp.r * A(kplus1,j).r - temp.i * A(kplus1,j).i, 
				    z__1.i = temp.r * A(kplus1,j).i + temp.i * A(kplus1,j).r;
...
/* L140: */
		...
		for (j = 1; j <= *n; ++j) {
			...
			if (nounit) {
			    i__4 = j * a_dim1 + 1;
			    z__1.r = temp.r * A(1,j).r - temp.i * A(1,j).i, 
				    z__1.i = temp.r * A(1,j).i + temp.i * A(1,j).r;
			    temp.r = z__1.r, temp.i = z__1.i;
			}
/* Computing MIN */
			i__1 = *n, i__2 = j + *k;
