...
/*        Form  C := alpha*A*B + beta*C. */
	...
	if (upper) {
		...
		for (i = 1; i <= *m; ++i) {
			...
			i__4 = k + j * b_dim1;
			i__5 = k + i * a_dim1;
			z__2.r = B(k,j).r * A(k,i).r - B(k,j).i * A(k,i)
				.i, z__2.i = B(k,j).r * A(k,i).i + B(k,j)
				.i * A(k,i).r;
...
/* L50: */
			...
			i__3 = i + j * c_dim1;
			i__4 = i + i * a_dim1;
			z__2.r = temp1.r * A(i,i).r - temp1.i * A(i,i).i, 
				z__2.i = temp1.r * A(i,i).i + temp1.i * A(i,i).r;
			...
			i__5 = i + i * a_dim1;
			z__4.r = temp1.r * A(i,i).r - temp1.i * A(i,i).i, 
				z__4.i = temp1.r * A(i,i).i + temp1.i * A(i,i).r;
...
/*        Form  C := alpha*B*A + beta*C. */
	...
	for (j = 1; j <= *n; ++j) {
	    i__2 = j + j * a_dim1;
	    z__1.r = alpha->r * A(j,j).r - alpha->i * A(j,j).i, z__1.i = 
		    alpha->r * A(j,j).i + alpha->i * A(j,j).r;
	    temp1.r = z__1.r, temp1.i = z__1.i;
	    if (beta->r == 0. && beta->i == 0.) {
		i__2 = *m;
		for (i = 1; i <= *m; ++i) {
...
/* L120: */
		...
		if (upper) {
		    ...
		    i__3 = j + k * a_dim1;
		    z__1.r = alpha->r * A(j,k).r - alpha->i * A(j,k).i, 
			    z__1.i = alpha->r * A(j,k).i + alpha->i * A(j,k)
			    .r;
		    temp1.r = z__1.r, temp1.i = z__1.i;
		}
		i__3 = *m;
