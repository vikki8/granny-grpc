	...
	integer *incx, complex *beta, complex *y, integer *incy)
{
    ...
    static integer info;
    static complex temp;
    static integer lenx, leny, i, j, k;
...
/* Computing MIN */
			...
			i__3 = i;
			i__5 = k + i + j * a_dim1;
			q__2.r = temp.r * A(k+i,j).r - temp.i * A(k+i,j).i, 
				q__2.i = temp.r * A(k+i,j).i + temp.i * A(k+i,j)
				.r;
...
/* Computing MIN */
			...
			i__3 = k + i + j * a_dim1;
			i__4 = i;
			q__2.r = A(k+i,j).r * X(i).r - A(k+i,j).i * X(i)
				.i, q__2.i = A(k+i,j).r * X(i).i + A(k+i,j)
				.i * X(i).r;
...
/* Computing MIN */
			...
			i__4 = k + i + j * a_dim1;
			i__2 = ix;
			q__2.r = A(k+i,j).r * X(ix).r - A(k+i,j).i * X(ix)
				.i, q__2.i = A(k+i,j).r * X(ix).i + A(k+i,j)
				.i * X(ix).r;
			q__1.r = temp.r + q__2.r, q__1.i = temp.i + q__2.i;
			temp.r = q__1.r, temp.i = q__1.i;
			ix += *incx;
