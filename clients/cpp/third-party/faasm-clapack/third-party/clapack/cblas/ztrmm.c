...
/*           Form  B := alpha*A*B. */
		...
		for (j = 1; j <= *n; ++j) {
			...
			if (B(k,j).r != 0. || B(k,j).i != 0.) {
			    i__3 = k + j * b_dim1;
			    z__1.r = alpha->r * B(k,j).r - alpha->i * B(k,j)
				    .i, z__1.i = alpha->r * B(k,j).i + 
				    alpha->i * B(k,j).r;
				...
				i__5 = i + j * b_dim1;
				i__6 = i + k * a_dim1;
				z__2.r = temp.r * A(i,k).r - temp.i * A(i,k)
					.i, z__2.i = temp.r * A(i,k).i + 
					temp.i * A(i,k).r;
...
/* L30: */
			    ...
			    if (nounit) {
				i__3 = k + k * a_dim1;
				z__1.r = temp.r * A(k,k).r - temp.i * A(k,k)
					.i, z__1.i = temp.r * A(k,k).i + 
					temp.i * A(k,k).r;
				temp.r = z__1.r, temp.i = z__1.i;
			    }
			    i__3 = k + j * b_dim1;
...
/* L50: */
		...
		for (j = 1; j <= *n; ++j) {
			...
			if (B(k,j).r != 0. || B(k,j).i != 0.) {
				...
				i__3 = k + j * b_dim1;
				i__4 = k + k * a_dim1;
				z__1.r = B(k,j).r * A(k,k).r - B(k,j).i * 
					A(k,k).i, z__1.i = B(k,j).r * A(k,k).i + B(k,j).i * A(k,k).r;
...
. */
		...
		for (j = 1; j <= *n; ++j) {
			...
			if (noconj) {
			    if (nounit) {
				i__2 = i + i * a_dim1;
				z__1.r = temp.r * A(i,i).r - temp.i * A(i,i)
					.i, z__1.i = temp.r * A(i,i).i + 
					temp.i * A(i,i).r;
				temp.r = z__1.r, temp.i = z__1.i;
			    }
			    i__2 = i - 1;
...
/*           Form  B := alpha*B*A. */
		...
		for (j = *n; j >= 1; --j) {
		    ...
		    if (nounit) {
			i__1 = j + j * a_dim1;
			z__1.r = temp.r * A(j,j).r - temp.i * A(j,j).i, 
				z__1.i = temp.r * A(j,j).i + temp.i * A(j,j)
				.r;
			temp.r = z__1.r, temp.i = z__1.i;
		    }
		    i__1 = *m;
			...
			z__1.r = temp.r * B(i,j).r - temp.i * B(i,j).i, 
				z__1.i = temp.r * B(i,j).i + temp.i * B(i,j)
				.r;
			B(i,j).r = z__1.r, B(i,j).i = z__1.i;
/* L170: */
		    }
			...
			if (A(k,j).r != 0. || A(k,j).i != 0.) {
			    i__2 = k + j * a_dim1;
			    z__1.r = alpha->r * A(k,j).r - alpha->i * A(k,j)
				    .i, z__1.i = alpha->r * A(k,j).i + 
				    alpha->i * A(k,j).r;
			    temp.r = z__1.r, temp.i = z__1.i;
			    i__2 = *m;
			    for (i = 1; i <= *m; ++i) {
...
. */
		...
		for (k = 1; k <= *n; ++k) {
			...
			if (A(j,k).r != 0. || A(j,k).i != 0.) {
			    if (noconj) {
				i__3 = j + k * a_dim1;
				z__1.r = alpha->r * A(j,k).r - alpha->i * A(j,k).i, z__1.i = alpha->r * A(j,k)
					.i + alpha->i * A(j,k).r;
				temp.r = z__1.r, temp.i = z__1.i;
			    } else {
				d_cnjg(&z__2, &A(j,k));
...
/* L260: */
			...
			if (noconj) {
			    i__2 = k + k * a_dim1;
			    z__1.r = temp.r * A(k,k).r - temp.i * A(k,k).i, 
				    z__1.i = temp.r * A(k,k).i + temp.i * A(k,k).r;
			    temp.r = z__1.r, temp.i = z__1.i;
			} else {
			    d_cnjg(&z__2, &A(k,k));
			    z__1.r = temp.r * z__2.r - temp.i * z__2.i, 
